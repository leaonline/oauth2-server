...
export class OAuth2Server {
   ...
   * @return {OAuth2Server}
   */
  constructor ({ serverOptions = {}, model, routes, debug, checkModelInterface = true } = {}) {
    ...
    if (!checkModelInterface || isModelInterface(model)) {
  ...
  authenticatedRoute () {
    ...
    return {
      ...
      post (route, fn) {
        app.post(route, debugMiddleware)
        app.post(route, authHandler)
        app.post(route, secureHandler(self, fn))
      }
    }
  }
  ...
  generateAccessTokenResponse () {
    ...
    self.route('post', accessTokenUrl, function (req, res, next) {
      ...
      if (req.body.grant_type === 'authorization_code') {
        const validParams = self.validateRequestParams(req, res, requiredAccessTokenPostParams)
        if (!validParams) return
      }

      if (req.body.grant_type === 'refresh_token') {
        const validParams = self.validateRequestParams(req, res, requiredRefreshTokenPostParams)
        if (!validParams) return
      }
  ...
  validateRequestParams (req, res, expectedParams) {
    const self = this
    const params = req.method.toLowerCase() === 'get' ? req.query : req.body

    if (!validateParams(params, expectedParams, self.debug)) {
      return errorHandler(res, {
        status: 400,
...
const validation = {
  ...
  requiredRefreshTokenPostParams, 

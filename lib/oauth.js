...
const publishAuthorizedClients = (pubName, debug) => {
  ...
  return Meteor.publish(pubName, function () {
    ...
    return Meteor.users.find(
      { _id: this.userId },
      { fields: { 'oauth.authorizedClients': 1 } }
    )
  })
}
...
export class OAuth2Server {
  ...
  constructor({ serverOptions = {}, model, routes, debug, logError } = {}) {
    ...
    const authorizedPubName =
      serverOptions?.authorizedPublicationName || 'authorizedOAuth'
  ...
  validateUser(fct) {
    ...
    UserValidation.register(this, fct)
  }
  ...
  authorizeHandler(options) {
    return async (req, res, next) => {
      ...
      try {
        const code = await this.oauth.authorize(request, response, options)
...
const initRoutes = (
  self,
  { accessTokenUrl = '/oauth/token', authorizeUrl = '/oauth/authorize' } = {}
) => {
  ...
  const route = ({ method, url, description, handler }) => {
    ...
    switch (method) {
      ...
      case 'post':
        return app.post(url, ...handlers)
      default:
        return app.use(url, ...handlers)
    }
  }
  ...
  route({
    ...
    handler: async (req, res, next) => {
      ...
      if (
        !user ||
        !(await UserValidation.isValid(self, validateUserCredentials))
      ) {
        return errorHandler(res, {
          status: 400,
          error: 'access_denied',
  ...
  route({
    ...
    handler: async (req, res /*, next */) => {
      ...
      try {
        const code = await self.oauth.authorize(
          request,
          response,
          authorizeOptions
        )

...
export const saveAuthorizationCode = async (code, client, user) => {
  ...
  await collections.AuthCodes.upsertAsync(
    { authorizationCode },
    {
      authorizationCode,
      expiresAt,
...
export const revokeAuthorizationCode = async ({ authorizationCode }) => {
  const docCount = await collections.AuthCodes.countDocuments({
    authorizationCode
  })
  if (docCount === 0) {
    return true
  }
  const removeCount = await collections.AuthCodes.removeAsync({
    authorizationCode
  })

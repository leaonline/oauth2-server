...
describe('constructor', () => {
  it('can be instantiated without any parameter', () => {
    const server = new OAuth2Server()
    assert.isDefined(server)
    assert.deepEqual(
      server.config.serverOptions,
      OAuth2ServerDefaults.serverOptions
    )
  ...
  it('can be created with serverOptions', () => {
  ...
  it('throws if server options include properties that are not in schema', () => {
    assert.throws(() => {
      ;(() => new OAuth2Server({ serverOptions: { foo: 'bar' } }))()
    })
  })

  it('can be created with custom config for default model', () => {
  ...
  it('can be created with a custom model', () => {
    const model = {
  ...
  it('can be created with custom routes', () => {
    const routes = {
    ...
    assert.deepEqual(
      ...
      Object.assign({}, OAuth2ServerDefaults.routes, routes)
    )
  })
})

describe('integration tests of OAuth2 workflows', () => {
  describe('Authorization code workflow', () => {
    const routes = {
    ...
    beforeEach(async () => {
      ClientCollection = Mongo.getCollection(
        DefaultModelConfig.clientsCollectionName
      )
      ...
      it('returns an invalid_request on invalid redirect_uri', async () => {
        ...
        await get(routes.authorizeUrl, { params }, (res) => {
          ...
          assert.equal(
            ...
            `Invalid redirection uri ${invalidRedirectUri}`
          )
          assert.equal(res.data.state, params.state)
        })
      })
            ...
            (res) => {
              if (followRedirects) {
                ...
                const queryParamsRegex = new RegExp(
                  `code=.+&user=${user._id}&state=${params.state}`,
                  'g'
                )
    ...
    describe('Access Token Request', () => {
      ...
      it('returns an invalid_request error if the redirect uri is not correct', async () => {
    ...
    describe('Access Token Response', () => {
      it('issues an access token for a valid request', async () => {
        ...
        await authCodeServer.model.saveAuthorizationCode(code, clientDoc, {
          id: user._id
        })
